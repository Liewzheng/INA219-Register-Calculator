...
function update_reg_0x00() {
    ...
    reg_0x00_reset = (reg_0x00 >> 15 == 1) ? true : false;
    newText += `- (RST) Reset: ${reg_0x00_reset}<br>`

    reg_0x00_brng = (( (reg_0x00 & 0x2000) >> 13) == 1) ? "32V" : "16V";
    newText += `- (BRNG) Bus Voltage Range: ${reg_0x00_brng}<br>`;

    reg_0x00_pga_gain = Math.pow(2, ((reg_0x00 & 0x1800) >> 11));
    reg_0x00_pga_range = reg_0x00_pga_gain * 40;
    newText += `- PGA Gain: ${reg_0x00_pga_gain}<br> - PGA Range: +/-${reg_0x00_pga_range}mV<br>`;
    ...
    newText += `- Mode: ${reg_0x00_mode}<br>`;


    newText += `<br>Value of 0x00 register: 0x${reg_0x00.toString(16).toUpperCase()}`;

    document.getElementById('reg_0x00').innerHTML = newText;
}
